...
```{r}
count_unique_regions_get_count(list_nmddataframes, counts_and_names_list$unique_names)
```
...
The following table gives the number of unique regions with a count greater than or equal to 4:
```{r}
count_unique_regions_get_count(list_ridataframes, counts_and_names_list$unique_names)
```