...
def get_random_bed_ids(lengthdistribution, UTR_bed_file):
    ...
    bed_ids = [bed.chrom for bed in three_prime_UTRs]
    randomlist=[]
    ...
    return randomlist, three_prime_UTRs, bed_ids


def write_bed_output(outname, randomlist, three_prime_UTRs, lengthdistribution, bed_ids):
    ...
    with open(outname, 'w') as out:
        ...
        while i > 0:
            ...
            random_seq = three_prime_UTRs.filter(lambda interval: interval.chrom == random_key)[0]
            ...
            if length < UTR_length:
                ...
                out.write(random_key + '\t' + str(start) + '\t' + str(end) + '\n')
                i-=1 
            else:
...
if len(sys.argv) < 3:
        print('usage python BackgroundRegions_adapted.py in.bed UTR.bed out.bed')
else :
