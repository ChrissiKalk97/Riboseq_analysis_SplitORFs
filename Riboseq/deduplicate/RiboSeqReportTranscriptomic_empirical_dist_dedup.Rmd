...
#library("dplyr")
#library("UpSetR")
source("helper_functions_Riboseq_report_dedup.R")
```
...
```{r}
count_unique_regions_get_count(list_nmddataframes, counts_and_names_list$unique_names, path)
```
...
The following table gives the number of unique regions with a count greater than or equal to 4:
```{r}
count_unique_regions_get_count(list_ridataframes, counts_and_names_list$unique_names, path)
```