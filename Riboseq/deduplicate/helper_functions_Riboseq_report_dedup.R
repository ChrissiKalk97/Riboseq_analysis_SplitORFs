# packages_required <- c('glue', 'types', 'stringr', 'dplyr', 'UpSetR', 'knitr', 'lemon', 'cmapR', 'yaml')
...
calculate_background_threshold <- function(unique_region_type = ? character, input_path){
  ...
  for(i in directories){
    if(!(identical(list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_random_intersect_counts_relative_sorted.bed")), character(0)))){
      randomfiles=c(randomfiles,paste0(i,"/",list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_random_intersect_counts_relative_sorted.bed"))))
    }
  }
  print(randomfiles)
...
print_thresholds <- function(unique_region_type = ? character, background, input_path){
  ...
  for(i in directories){
    if(!(identical(list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_random_intersect_counts_relative_sorted.bed")), character(0)))){
      randomSetnames=c(randomSetnames,list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_random_intersect_counts_relative_sorted.bed")))
    }
  }
  randomnames=stringr::str_replace(randomSetnames, pattern = "_dups_removed_random_intersect_counts_relative_sorted.bed", replacement = "")
...
get_intersect_files <- function(unique_region_type = ? character, input_path){
  ...
  setnames=list()
  for(i in directories){
    if(!(identical(list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_intersect_counts_relative_sorted.bed")), character(0)))){
      files=c(files,paste0(i,"/",list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_intersect_counts_relative_sorted.bed"))))
      setnames=c(setnames,list.files(i,pattern=paste0("*", unique_region_type, "_dups_removed_intersect_counts_relative_sorted.bed")))
    }
  }
  ...
  dataframes=datalist
  names(dataframes) <- stringr::str_replace(setnames, pattern = "_dups_removed_intersect_counts_relative_sorted.bed", replacement = "")
...
count_unique_regions_get_count <- function(dataframes, unique_names_per_sample, path){
  relevantregionscount=list()
  frame_number = 1
  for(frame in dataframes){
    colnames(frame)=c("ID","start","stop","ORF","read_count",  "relative_count")
    ...
    write.csv(frame, paste(path, paste(names(dataframes)[frame_number],  "unique_regions.csv", sep = "_"), sep = '/'))
    above_5_counter=0
    c=1
    for(count in frame$relative_count){
