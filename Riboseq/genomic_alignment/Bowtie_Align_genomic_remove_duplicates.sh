...
STAR --runThreadN $numberOfThreads --alignEndsType EndToEnd --outSAMstrandField intronMotif --alignIntronMax 1 --genomeDir $StarIndex\
 --readFilesIn $Riboreads --outFileNamePrefix $out_path/$(basename $bamfile .bam)
...
samtools sort -o $bamfile $bamfile

#bowtie2 --threads $numberOfThreads -x $bowtieBaseName -U $Riboreads | samtools view -@ $numberOfThreads -bS | samtools sort -o $bamfile
samtools markdup -r -f $out_path/$(basename $bamfile .bam)_removedups.out $bamfile $out_path/$(basename $bamfile .bam)_dups_removed.bam
bamfile=$out_path/$(basename $bamfile .bam)_dups_removed.bam
bedfile=$out_path/$(basename $bamfile .bam).bed

echo "converting bam to bed"
bedtools bamtobed -bed12 -i $bamfile > $bedfile
sortedBedfile=$out_path/$(basename $bedfile .bed)_intersect_counts_sorted.bed

echo "intersecting with unique regions"
bedtools intersect -s -wao -F 0.33 -a $unique_regions -b $bedfile | sort -nr -k 4 > $sortedBedfile
