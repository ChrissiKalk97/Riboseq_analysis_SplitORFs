...
def get_random_bed_ids(lengthdistribution, UTR_bed_file):
    ...
    bed_names = list(set([bed.name for bed in three_prime_UTRs]))
    randomlist=[]
...
def write_bed_output(outname, randomlist, three_prime_UTRs, lengthdistribution, bed_names):
    ...
    with open(outname, 'w') as out:
        ...
        while i > 0:
            ...
            random_seqs = three_prime_UTRs.filter(lambda interval: interval.name == random_key).saveas()
            if len(random_seqs) == 0:
                ...
                interval_nr = random.choice(range(len(random_seqs)))
