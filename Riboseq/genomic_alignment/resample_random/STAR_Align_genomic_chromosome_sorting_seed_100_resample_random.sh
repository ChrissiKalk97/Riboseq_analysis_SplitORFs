...
for i in {1..10}; do
  ...
  python /home/fuchs/agschulz/kalk/scripts/SplitORFs/Riboseq/genomic_alignment/resample_random/BackgroundRegions_bed_genomic_seed_100_resample_random.py\
  ...
  while read -r line; do
    ...
    # Store the chromosome as a key in the associative array
    random_chromosomes["$chr"]=1
  done < $randomfile
  ...
  randomintersectfile=$out_path/$(basename $bamfile .bam)_${i}_random_intersect_counts.bed
  bedtools intersect\
    -s\
    -wao\
    -sorted\
