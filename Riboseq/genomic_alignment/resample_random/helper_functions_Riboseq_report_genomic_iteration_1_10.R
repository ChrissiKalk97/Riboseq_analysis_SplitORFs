...
calculate_background_threshold <- function(unique_region_type = `?`(character), path) {
    ...
    for (i in directories) {
        if (!(identical(list.files(i, pattern = paste0("*", unique_region_type,
            "_\\b(10|[1-9])\\b\\_random_intersect_counts.bed")), character(0)))) {
            randomfiles <- c(randomfiles, paste0(i, "/", list.files(i,
                pattern = paste0("*", unique_region_type, "_\\b(10|[1-9])\\b\\_random_intersect_counts.bed"))))
        }
    }
...
print_thresholds <- function(unique_region_type = `?`(character),
    ...
    randomSetnames <- list()
    for (i in directories) {
        if (!(identical(list.files(i, pattern = paste0("*", unique_region_type, "_\\b(10|[1-9])\\b\\_random_intersect_counts.bed")), character(0)))) {
    ...
    print(randomnames)
    printbackground$sample <- stringr::str_replace(rownames(printbackground), pattern = paste0("_", unique_region_type, "_\\b(10|[1-9])\\b"),
                                                   replacement = "")
