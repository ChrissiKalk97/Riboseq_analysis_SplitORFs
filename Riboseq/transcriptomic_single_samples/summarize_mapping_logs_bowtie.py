    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if "aligned 0 times" in line:
                sample_data["Unaligned (%)"] = line.split('(')[1].split('%')[0].strip()
            elif "aligned exactly 1 time" in line:
                sample_data["Aligned Once (%)"] = line.split('(')[1].split('%')[0].strip()
            elif "aligned >1 times" in line:
                sample_data["Aligned Multiple (%)"] = line.split('(')[1].split('%')[0].strip()
            elif "overall alignment rate" in line:
                sample_data["Overall Alignment Rate (%)"] = line.split('%')[0].strip()
...
def write_to_csv(data, output_path):
    fieldnames = ["Sample", "Unaligned (%)", "Aligned Once (%)", "Aligned Multiple (%)", "Overall Alignment Rate (%)"]
